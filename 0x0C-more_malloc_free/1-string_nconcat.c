 ...
 * @n: an integer (6) with the limit of what to be added to s1
 *
 * Return: pointer to a newlly allocated memory with s1 & s2, NULL if it fail
 */
char *string_nconcat(char *s1, char *s2, unsigned int n)
{
	/* unsigned int var since we need to compare with n */
	char *strings;
	...
	if (s2 == NULL)
		s2 = "";
	/* go through both strings so it can find its length */
	...
	/* Since the length of s2 is known, we can compare it with n */
	/* set the task condition */
	if (n > lengths2)
		n = lengths2;
